clc; clear; close all;
groupname = "Group17";
fileName = './'+groupname+'/Calibration_%03d.txt'; 
%%
option = 3; 
if (option == 1)
    load('Signal1');
elseif (option == 2)
    load('Signal2');
elseif (option == 3)
    fileIn = '.\'+groupname+'\CorrelationTest.txt';
...
velocity = polyval(c,u);

u_mean = mean(velocity(1:nt));
u_std  = std(velocity(1:nt));

Ti = 1/fi;
epsilon = 0.01;
coverage = 3;

N = round((coverage*u_std/u_mean/epsilon)^2);
